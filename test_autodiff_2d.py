...
def train_second_order(dim=1, tol=1e-3, max_iter=10000, size_layer=256, n_layers=3):
    ...
    for i in (pbar:= tqdm(range(int(max_iter)))):
        ...
        loss = torch.linalg.norm(laplacian_pred - true, ord=2)**2 + 100*(torch.linalg.norm(pred_b1 - f_2d(b1[:, 0], b1[:, 1]), 2)**2 + torch.linalg.norm(pred_b2 - f_2d(b2[:, 0], b2[:, 1]), 2)**2 + torch.linalg.norm(pred_b3 - f_2d(b3[:, 0], b3[:, 1]), 2)**2 + torch.linalg.norm(pred_b4 - f_2d(b4[:, 0], b4[:, 1]), 2)**2)
...
def plot():
    ...
    for file in filenames:
        ...
        loss_l2 = np.linalg.norm(true - pred, 2)
    ...
    plt.loglog(x, fem_losses_sorted, color='purple', label="FEM")
